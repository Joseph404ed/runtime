...
impl Agent for AskerAgent {
    ...
    async fn execute(&mut self, ctx: &AgentContext) -> AgentResult<()> {
        if !self.asked {
            println!("  [Asker] → Sending: \"What patterns does RustyAI support?\"");
...
/// An agent that answers questions about RustyAI
...
impl Agent for ResponderAgent {
    ...
    async fn handle_message(
        ...
        // Reply
        let answer = "RustyAI supports 8 patterns: Hierarchy, Swarm, Coalition, Market, Federation, Team, Holarchy, and Blackboard.";
...
async fn main() -> Result<(), RuntimeError> {
    println!("╔══════════════════════════════════════════╗");
    println!("║   RustyAI — Agents Talking Demo       ║");
    println!("╚══════════════════════════════════════════╝\n");
